            ...
            HelpSection(
                ...
                items: [
                    ...
                    HelpItem(term: t("Stations-Schildchen", "Station tags"), symbol: "tag.fill",
                             detail: t("Alle Stationen im sichtbaren Bereich erscheinen als beschriftete Schildchen entlang der Skala. Ein Klick darauf stimmt direkt auf diese Frequenz ab. Liegen mehrere Stationen exakt auf derselben Frequenz, zeigt die Skala nur so viele gleichzeitig, wie vertikal Platz haben.",
                                       "Every station within the visible span appears as a labelled tag along the dial. Click one to tune straight to that frequency. When several stations sit on the exact same frequency, the dial only shows as many at once as fit vertically.")),
                    HelpItem(term: t("Zoom", "Zoom"), symbol: "plus.magnifyingglass",
                             detail: t("Die beiden Lupen-Knöpfe oben rechts auf der Skala vergrößern bzw. verkleinern den angezeigten Frequenzbereich manuell – praktisch, wenn sich an einer Stelle zu viele Stationen drängen.",
                                       "The two magnifying-glass buttons at the top right of the dial widen or narrow the visible frequency span manually — handy when too many stations crowd one spot.")),
                    ...
                    HelpItem(term: t("Tastatur", "Keyboard"), symbol: "keyboard.fill",
                             detail: t("← / → stimmt in 1-kHz-Schritten fein ab, ↑ / ↓ springt zur nächsten bzw. vorherigen Station in der Liste. Das funktioniert überall in der App, außer während du gerade in ein Textfeld tippst.",
                                       "← / → fine-tunes in 1 kHz steps, ↑ / ↓ jumps to the next/previous station in the list. This works anywhere in the app, except while you’re typing in a text field.")),
                ]),

            HelpSection(
                title: t("Stationsliste", "Station list"),
                icon: "list.bullet.rectangle.fill",
                intro: t("Rechts neben der Skala zeigt die Liste alle Stationen des geladenen Sendeplans – einzeilig, damit mehr auf einen Blick passen. Du kannst frei durch das gesamte Spektrum scrollen; beim Abstimmen zentriert sich die Liste automatisch auf die gerade empfangene Frequenz.",
                         "To the right of the dial the list shows every station in the loaded schedule — one line each, so more fit on screen at once. You can freely scroll through the whole spectrum; when you tune, the list automatically re-centres on the frequency you're receiving."),
                items: [
                    HelpItem(term: t("Weiß hervorgehoben", "Highlighted white"), symbol: "circle.fill",
                             detail: t("Diese Station liegt exakt auf der eingestellten Frequenz, sendet aber gerade nicht.",
                                       "This station sits exactly on the tuned frequency, but isn’t on the air right now.")),
                    HelpItem(term: t("Gelb hervorgehoben", "Highlighted yellow"), symbol: "circle.fill",
                             detail: t("Die Frequenz stimmt UND die Station sendet laut Sendeplan gerade jetzt – die ganze Zeile leuchtet statt Weiß in Gelb.",
                                       "The frequency matches AND the station is on the air right now according to the schedule — the whole row glows yellow instead of white.")),
                    HelpItem(term: t("„ON AIR“-Hinweis", "“ON AIR” badge"), symbol: "dot.radiowaves.left.and.right",
                             detail: t("Erscheint bei jeder Station, die laut Sendeplan gerade sendet (Uhrzeit und Wochentag stimmen) – unabhängig davon, ob sie gerade eingestellt ist. Die Bewertung erfolgt in UTC.",
                                       "Appears on every station that’s on the air right now according to the schedule (time and weekday match) — regardless of whether it’s the tuned one. Evaluation is in UTC.")),
